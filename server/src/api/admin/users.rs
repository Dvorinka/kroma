...
pub async fn list_users(
    ...
    let locale = user.language.as_deref().and_then(kroma_engine::i18n::normalize).unwrap_or(req_locale).to_string();
    let (mut users, library_count) =
        query(&state.db, move |pool| Ok((db::admin_users(&pool, &locale)?, db::counts(&pool)?.0))).await?;
    for u in &mut users {
